...
Public Class frm_sp_rpt_Invoice
    ...
    Private Sub frm_sp_rpt_Invoice_FormClosing(sender As Object, e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        If bw1.IsBusy = True AndAlso bw1.CancellationPending = False Then
            bw1.CancelAsync()
        End If
    End Sub
    Private Sub frm_sp_rpt_Invoice_Load(sender As System.Object, e As System.EventArgs) Handles Me.Load
        Try
            Dim f As New ByteClassLibrary.MyFonts
            f.FontMyControl(Me)
...
#Region "       Subs and Functions"
    ...
    Private Sub FormatSumRow()
        If MyBs.Find("IsSum", 1) >= 0 Then
            ...
            Dim FontName As String = dgv.DefaultCellStyle.Font.FontFamily.Name



            Dim f As New Font(FontName, dgv.DefaultCellStyle.Font.Size, FontStyle.Bold)
    ...
    Private Function fn_sp_rpt_Invoice(ByRef InvoiceID As Object, ByRef InvoiceTypeID As Object, ByRef IsPaid As Object, ByRef InvoiceNumber As Object, ByRef InvoiceRealNumber As Object, ByRef OrgInvoiceID As Object, ByRef OrgInvoiceNumber As Object, ByRef InvDate As Object, ByRef AgentID As Object, ByRef DistributorID As Object, ByRef Name As Object, ByRef TotalPrice As Object, ByRef FinalPrice As Object, ByRef Revenue As Object, ByRef Note As Object, ByRef UserID As Object, ByRef InvoiceInfo As Object, ByRef AgentName As Object, ByRef DistributorName As Object, ByRef Username As Object, ByRef FullName As Object, ByRef WareHouseID As Object, ByRef WareHouseName As Object) As DataTable
...
#Region "       Events"
    ...
    Private Sub frm_sp_rpt_Invoice_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub btnSearch_Click(sender As System.Object, e As System.EventArgs) Handles btnSearch.Click
        ...
        End Try
    End Sub
    Private Sub dgv_Sorted(sender As Object, e As System.EventArgs) Handles dgv.Sorted
    ...
    Private Sub bw1_DoWork(sender As System.Object, e As System.ComponentModel.DoWorkEventArgs) Handles bw1.DoWork
        ...
        Try
            ...
            MyTable1 = fn_sp_rpt_Invoice(InvoiceID, InvoiceTypeID, IsPaid, InvoiceNumber, InvoiceRealNumber, OrgInvoiceID, OrgInvoiceNumber, InvDate, AgentID, DistributorID, Name, TotalPrice, FinalPrice, Revenue, Note, UserID, InvoiceInfo, AgentName, DistributorName, Username, FullName, WareHouseID, WareHouseName)
        ...
        Try
            ...
            If IsBW1Canseled = False Then
                If req = RequestType.dgv Then
                    ...
                    dgv.Columns("WareHouseID").HeaderText = "رقم المخزن"
                    dgv.Columns("WareHouseName").HeaderText = "المخزن"
                    dgv.Columns("PlusOrMinus").HeaderText = "pm"
                    dgv.Columns("StockID").HeaderText = "ر.م"
                    dgv.Columns("StockName").HeaderText = "البيان"
                    dgv.Columns("ProductDetails").HeaderText = "التفاصيل"
                    dgv.Columns("Barcode").HeaderText = "باركود"
                    dgv.Columns("Amount").HeaderText = "الكمية"
                    dgv.Columns("UnitPrice").HeaderText = "السعر"
                    dgv.Columns("UnitPriceCost").HeaderText = ""
                ...
                Else
                    ...
                    With frm.rvReport
                        .LocalReport.ReportEmbeddedResource = My.Application.Info.AssemblyName & ".rpt_sp_rpt_Invoice.rdlc"
                        ...
                        If IsDBNull(Name) = False Then
                            .LocalReport.SetParameters(New ReportParameter("Name", Name.ToString, True))
                        End If
                        ...
                        If IsDBNull(InvoiceInfo) = False Then
                            .LocalReport.SetParameters(New ReportParameter("InvoiceInfo", InvoiceInfo.ToString, True))
                        End If
                        ...
                        If IsDBNull(FullName) = False Then
                            .LocalReport.SetParameters(New ReportParameter("FullName", FullName.ToString, True))
                        End If
                        ...
                        Dim img As Image
                        img = ByteClassLibrary.MyFunctions.GenBarcodeImageNoLabel(Format(InvoiceNumber, "000"), My.Application.Info.DirectoryPath & "\per\TempInvoiceNumber.jpg")
                        '      img = ByteClassLibrary.MyFunctions.GenBarcodeImage("5812", My.Application.Info.DirectoryPath & "\per\TempInvoiceNumber.jpg")


                        .LocalReport.SetParameters(New ReportParameter("ImageInvoiceBarcode", "file:///" & My.Application.Info.DirectoryPath & "\per\TempInvoiceNumber.jpg", True))
                        .LocalReport.SetParameters(New ReportParameter("FinalPriceString", CurrencyToString(FinalPrice), True))


                        Dim Discount As Decimal
                        If TotalPrice = 0 Then Discount = 0 Else Discount = 1 - (FinalPrice / TotalPrice)
